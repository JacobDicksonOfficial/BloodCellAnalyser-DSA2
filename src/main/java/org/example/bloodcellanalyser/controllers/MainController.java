...
public class MainController implements Initializable {
    ...
    // Add to the MainController class (public section)
    ...
    public void setImageArray(int[] cellArray) {
        this.cellArray = cellArray;
    }

    public void setPills(HashMap<Integer, BloodCellDetails> map) {
        this.cells = map;
    }
    ...
    public void convertToTricolor() {
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                ...
                double hue = c.getHue(), sat = c.getSaturation(), bright = c.getBrightness();

                int index = y * width + x;
                if (hue > 300 || (hue < 20 && bright > 0.6)) { // Red blood cells
                    tricolor.getPixelWriter().setColor(x, y, Color.RED);
                    cellArray[index] = -1;
                } else if (hue > 200 && sat > 0.3) { // Purple nuclei (white blood cells)
    ...
    public void analyzeCells() {
        // Union-Find red and purple clusters
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                int idx = y * width + x;
                if (cellArray[idx] < 0) {
                    if (x + 1 < width && cellArray[y * width + (x + 1)] == cellArray[idx])
                        uf.union(idx, y * width + (x + 1));
                    if (y + 1 < height && cellArray[(y + 1) * width + x] == cellArray[idx])
        ...
        for (Map.Entry<Integer, List<Integer>> entry : clusters.entrySet()) {
            ...
            if (cellArray[indices.get(0)] == -2) {
                ...
                whiteCount++;
                type = "white";
            } else if (size < 60) { // Single red cell
                rectColor = Color.GREEN;
                redCount++;
                type = "red";
            } else { // Cluster
                rectColor = Color.BLUE;
                redCount += Math.round(size / 50.0); // Estimate
                type = "red-cluster";
            }
